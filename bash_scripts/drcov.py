...
def read_nm_log():
    with open("jsc_nm_entries.txt", "r") as ifd:
        nm = {}
...
if __name__ == "__main__":
    ...
    nm = read_nm_log()
    if not nm:
        print "nm failed to read"
        sys.exit()
     
    # attempt file parse
    x = DrcovData(argv[1])
    blocks = x.get_blocks_by_module("jsc")
    #print len(blocks)
    for start,size in blocks:
    #    print "%d- %d" % (start, size) 
        start = start + 0x400000
        if start in nm:
            print "0x%08x -> %s" % (start, nm[start])
